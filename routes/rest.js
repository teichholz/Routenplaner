...
restRoutes.get("/key/google", function (req, res) {
    google.readGoogleApiKey()
        ...
        .catch(function (err) {
            res.send({
                err: "key konnte nicht geladen werden"
            });
            res.end();
        });
});
...
restRoutes.get("/poi/get", function (req, res) {
    ...
    let lat = req.query.latitude || 0;
    let lng = req.query.longitude || 0;
    console.log(radius);

    //benutzt default felder
    let prom = g.fetchGoogleResult(g.googleNearbySearchUrl(null, radius, {
        latitude: lat,
        longitude: lng
    }));
    prom
